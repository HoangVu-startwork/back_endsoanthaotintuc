...
const { getBlogByUrl, createIoFileHtmlEn, createIoFileHtmlVn, createIoFileHtmlBlogen, createIoFileHtmlBlogvn, invalidateAndIoDelete, filesIoExistInS3, invalidateAndIoDeleteVn } = require('../services/blog-ucademy')
// const { uploadfile } = require('../services/upload');

async function getIoBlog(req, res) {
    ...
    try {
      ...
      <head>
          ...
          <meta name="keywords"
              content="ucademy, learning, corporate, lms, online, marketing, saas, paas, training, startup, ucademy.io">
          <meta name="author" content="Ucademy">
          <meta name="og:site_name" content="Ucademy">
          <meta property="og:title" content="${blog.metaBlog}">
          <meta property="og:url" content="https://ucademy.io/blog/${blog.urlBlog}.html">
          ...
          <title>${blog.titleBlog} || ucademy.io</title>
                          ...
                          <div class="widget">
                              ...
                              <div class="newsletter-wrapper">
                                  <div id="mc_embed_signup2">
                                      <form action="https://api.ucademy.io/newsletter" method="post"
  ...
  <body>
    ...
    `;
        ...
        if (blog.language === 'en') {
            ...
            filesIoExistInS3(fileNameEn, fileNameVn)
                .then(async ({ existsEn, existsVn }) => {
                    if (existsEn) {
                        // If English file exists, invalidate and delete
                        await invalidateAndIoDelete(fileNameEn);
                    }
        
                    if (existsVn) {
                        // If Vietnamese file exists, invalidate and delete
                        await invalidateAndIoDeleteVn(fileNameVn);
                    }
        
                    // Create new files
                    try {
                        const [fileHtmlEn, fileHtmlVn] = await Promise.all([
                            createIoFileHtmlEn(req.file, fileNameEn, htmldist),
                            createIoFileHtmlVn(req.file, fileNameVn, htmlvn)
                        ]);
        ...
        if (blog.language === 'vn') {
            ...
            filesIoExistInS3(fileNameEn, fileNameVn)
                .then(async ({ existsEn, existsVn }) => {
                    ...
                    try {
                        const [fileHtmlEn, fileHtmlVn] = await Promise.all([
                            createIoFileHtmlBlogen(req.file, fileNameEn, htmlblogviewdist),
                            createIoFileHtmlBlogvn(req.file, fileNameVn, htmldistblog)
                        ]);
